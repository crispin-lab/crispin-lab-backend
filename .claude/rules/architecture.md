...
- `lab-common-persistence` — Exposed 어댑터 공용 base + Testcontainers/Flyway test helper. main 에 `ExposedEntityRepository<E, I>` (`com.crispinlab.common.persistence`), testFixtures 에 `PostgresTestContext` / `TestcontainersConfig`. `crispinlab.kopring.library + crispinlab.kopring.exposed + java-test-fixtures` 플러그인. `api(labCommonDomain)` 만 노출하고 Exposed / spring-boot-test / Testcontainers / Flyway 의존은 모두 `testFixturesApi(...)` 로 testFixtures consumer 에게만 전파. 도메인 `app` 모듈이 `implementation(projects.labCommonPersistence)` + `testImplementation(testFixtures(projects.labCommonPersistence))` 로 받는다.
