...
- `lab-common-persistence` — Exposed 어댑터 공용 base + Testcontainers/Flyway test helper. main 에 `ExposedEntityRepository<E, I>` (`com.crispinlab.common.persistence`), testFixtures 에 `PostgresTestContext` / `TestcontainersConfig`. `crispinlab.kopring.library + crispinlab.kopring.exposed + java-test-fixtures` 플러그인. `api(labCommonDomain)` 만 명시 노출. main 의 Exposed 의존은 `crispinlab.kopring.exposed` 컨벤션 플러그인이 `implementation` 으로 주입 (`ExposedEntityRepository` 가 main 에서 Exposed 타입을 직접 쓰므로 main scope 필수). testFixtures 는 main 의 `implementation` 을 inherit 받지 않으므로 spring-boot-test / Testcontainers / Flyway 와 Exposed 를 함께 `testFixturesApi(...)` 로 재명시해 testFixtures consumer 에게 전파. 도메인 `app` 모듈이 `implementation(projects.labCommonPersistence)` + `testImplementation(testFixtures(projects.labCommonPersistence))` 로 받는다.
