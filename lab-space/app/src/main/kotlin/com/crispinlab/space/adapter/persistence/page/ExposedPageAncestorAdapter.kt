    ...
    companion object {
        ...
        private val ANCESTOR_CHAIN_SQL =
            ...
            WITH RECURSIVE ancestor_chain AS (
                SELECT p.id, p.title, p.parent_page_id, p.visibility, p.space_id, p.author_id, 1 AS depth
                FROM pages p
                ...
                SELECT p.id, p.title, p.parent_page_id, p.visibility, p.space_id, p.author_id, ac.depth + 1
                FROM pages p
            ...
            SELECT id, title, visibility, space_id, author_id
            FROM ancestor_chain
            ORDER BY depth DESC
            """.trimIndent()
    }
